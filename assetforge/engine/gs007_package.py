...
class GS007Package:
    ...
    def execute(self, state: AssetForgeState) -> AssetForgeState:
        ...
        self._ensure_iteration_documents(state, iteration_root)
    ...
    @staticmethod
    def _ensure_iteration_documents(state: AssetForgeState, iteration_root: Path) -> None:
        manifest = state.configs.get("Manifest.yaml", {})
        iteration_name = manifest.get("iteration", {}).get("name", f"Iteration {state.iteration:02d}")
        iteration_root.mkdir(parents=True, exist_ok=True)
        readme = iteration_root / "Readme.md"
        if not readme.exists():
            readme.write_text(
                f"# Iteration {state.iteration:02d} — {iteration_name}\n\n"
                "Generated by AssetForge.\n",
                encoding="utf-8",
            )
        report_directory = iteration_root / "Production_Report"
        report_directory.mkdir(parents=True, exist_ok=True)
        report = report_directory / "Production_Report.md"
